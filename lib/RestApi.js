...
var NorenRestApi = function (params) {
  ...
  axios.interceptors.request.use((req) => {
    // console.log(`${req.method} ${req.url} ${req.data}`); // Comment by rahul
  ...
  axios.interceptors.response.use(
    (response) => {
      if (API.debug) {
        //console.log(response); // commented by rahul
      }
      ...
      if (response.status === 200) {
        if (response.data.success || response.data.status) {
          ...
          return response.data;
          // return { login: "successfully received data" };
        }
      }
    },
    (error) => {
  ...
  self.login = function (params) {
    ...
    // console.log(authparams); // Added and commented by Rahul
